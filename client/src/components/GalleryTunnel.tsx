...
export default function GalleryTunnel(props: GalleryTunnelProps) {
  const {
    ...
    lineColor = "#1a1a2e",
    lineOpacity = 30,
    colors = ["#00d4aa", "#0099cc", "#006699", "#004466", "#002233", "#00d4aa"],
    grid = 3,
    speed = 50,
    boost = 80,
    fade = 100,
    label = true,
    labelText = "Scroll",
    labelFill = "rgba(0,212,170,0.15)",
    labelColor = "#00d4aa",
    labelFont = { fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 400 },
    style,
    className = "",
  } = props;
  ...
  const palette = useMemo(() => {
    ...
    return list.length ? list : ["#00d4aa", "#0099cc"];
  }, [colors]);
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      for (let i = fading.length - 1; i >= 0; i--) {
        const m = fading[i];
        m.opacity = Math.min(0.15, m.opacity + dt / FADE_IN);
      }
  ...
  }, [palette, background, lineColor, lineOpacity, grid, fade]);
